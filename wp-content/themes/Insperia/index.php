...
<section class="splash-banner page-header-wrap parallax">
	<div class="section pattern-wrap color1">
		<div class="container">
			<h1 class="page-header h1 wow fadeInUp"><?php echo esc_attr(of_get_option('index_title',$prof_default) ); ?></h1>
			<?php
				if(of_get_option('index_sub_title',$prof_default) != ''){ ?>
					<p class="page-sub-title wow fadeInUp"><?php echo esc_attr(of_get_option('index_sub_title',$prof_default)); ?></p>
			<?php } ?>
			<ol class="breadcrumb">
				<li> <a href="<?php echo esc_url(home_url()); ?>"> <i class="fa fa-home"></i> </a> </li>
				<li class="active"><?php echo esc_attr(of_get_option('index_title',$prof_default) ); ?></li>
			</ol>
		</div>
	</div>
</section>
						...
						if ( get_post_format() == false && has_post_thumbnail()) {
							...
							$return_string .= $getText;
							 ...
							 $return_string .='<header class="post-header">
												...
												<small>' . esc_attr(get_post_meta(get_the_ID(), 'Quote Person Name', true)) . ' ' . esc_attr($insting) . ' <cite title="' . esc_attr(get_post_meta(get_the_ID(), 'Post Audio Shortcode', true)) . '">' . esc_attr(get_post_meta(get_the_ID(), 'Quote Person Company', true)) . '</cite></small> </blockquote>
