...
class wc_Widget_Brand_Slider extends WP_Widget {
	/**
	 * Constructor
	 */
	public function __construct(){
		$widget_options = array( 'classname' => 'wc_widget_brand_slider', 'description' => __( 'Shows product brans with slider','luxury-wp') );
		$control_options = array();
		parent::__construct('wc_widget_brand_slider',__('DHWOO Product Brand Slider','luxury-wp'), $widget_options, $control_options);
		
		if (!is_admin()){
			add_action("wp_print_styles", array(&$this, 'enqueue_scripts'));
			wp_enqueue_script('wc-widget-brand-slider',DHINC_URL.'/wc-brand/assets/js/jquery.bxslider.min.js',array('jquery'),'',true);
		}
		
	}
	
	function enqueue_scripts(){
		wp_enqueue_style('wc-widget-brand-slider',DHINC_URL.'/wc-brand/assets/css/jquery.bxslider.css');
	...
	public function widget($args, $instance){
		...
		<div class="product-brand-slide">
			<ul id="pb-<?php echo $args['widget_id']?>" class="list-brand">
				...
				<?php 
				$thumbnail = wc_get_product_brand_thumbnail_url( $brand->term_id);
				if ( ! $thumbnail )
