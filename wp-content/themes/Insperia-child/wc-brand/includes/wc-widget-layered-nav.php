...
class WC_Widget_Layered_Nav extends WP_Widget {
	...
	public function __construct(){
		$widget_options = array( 'classname' => 'woocommerce widget_layered_nav wc_widget_layered_nav', 'description' => __( 'Shows product brands in a widget which lets you narrow down the list of products when viewing product categories','luxury-wp') );
		$control_options = array();
		parent::__construct('wc_widget_layered_nav',__('Product Brand Layered Nav','luxury-wp'), $widget_options, $control_options);
	}
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				foreach ( $terms as $term ) {
					...
					echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'wc_layered_nav_link', $link ) ) . '">' : '<span>';
