...
define('MHTRP_VERSION', '1.8.1');
define('MHTRP_CLASS_ANY', '_any');
define('MHTRP_CLASS_NONE', '_none');
define('MHTRP_DB_VERSION', 1);
		...
		if ( ! class_exists( 'MH_Table_Rate_Plus_Shipping_Method' ) ) {
			class MH_Table_Rate_Plus_Shipping_Method extends WC_Shipping_Method {
				...
				function init() {
					...
					$this->class_mapping = array('_none' => '-1', '_any' => '0');

					// Set tax mode if not defined
					if ( empty( $this->settings['tax_mode'] ) ) {
						$this->tax_mode = 'excl';
					} else {
						$this->tax_mode = $this->settings['tax_mode'];
					}
					...
					$this->form_fields = array(
						...
						'tax_status' => array(
							...
							'options'		=> array(
								...
								'none' 		=> __( 'Not taxable', MHTRP_DOMAIN ),
							),
						),
						'tax_mode' => array(
							'title' 		=> __( 'Tax Mode', MHTRP_DOMAIN ),
							'description' 		=> __( 'Shipping costs and conditions can be entered exclusive or inclusive of tax.', MHTRP_DOMAIN ),
							'type' 			=> 'select',
							'default' 		=> 'excl',
							'options'		=> array(
								'excl' 	=> __( 'Values exclude tax', MHTRP_DOMAIN ),
								'incl' 		=> __( 'Values include tax', MHTRP_DOMAIN ),
							),
							'desc_tip'     => true,
						),
	                    'handling_fee' => array(
                            'title' 		=> __( 'Handling Fee', MHTRP_DOMAIN ),
                            'description' 		=> __( 'Optional handling fee. Leave blank to disable.', MHTRP_DOMAIN ),
                            'type' 			=> 'price',
                            'default' 		=> '0.00',
                            'desc_tip'     => true,
						),
						'calc_type' => array(
							...
							'description' 		=> __( '<strong>Per order</strong> - calculated on whole cart, using the highest priority shipping class.<br><br><strong>Per shipping class</strong> - calculated for each shipping class, then added together.<br><br><strong>Highest priority shipping class</strong> - calculated on highest priority shipping class only.', MHTRP_DOMAIN ),
							'type' 			=> 'select',
							'default' 		=> 'per_order',
							'options'		=> array(
								'per_order' 	=> __( 'Per order', MHTRP_DOMAIN ),
								'per_class' 	=> __( 'Per shipping class', MHTRP_DOMAIN ),
								'highest_class' 	=> __( 'Highest priority shipping class', MHTRP_DOMAIN ),
							),
							'desc_tip'     => true,
						),
				...
				function first_run() {
					...
					if ( get_option($this->first_run_option) === FALSE ) {
						...
						update_option( $this->first_run_option, MHTRP_DB_VERSION );
				...
				*/
				...
				?>
					...
					<style>
						...
						<?php if ( WC()->shipping->get_shipping_classes() == false ) { ?>
						.mhtrsp-class-col {
							display: none;
						}
						<?php } ?>
							...
							<table class="shippingrows widefat" cellspacing="0">
								<thead>
									<tr>
										...
										<th class="mhtrsp-class-col"><?php _e( 'Class', MHTRP_DOMAIN ); ?> <a class="tips" data-tip="<?php _e( 'Shipping class, as defined in WooCommerce settings.', MHTRP_DOMAIN ); ?>">[?]</a></th>
										...
										<th><?php _e( 'Base Cost', MHTRP_DOMAIN ); ?> <a class="tips" data-tip="<?php _e( 'Flat rate cost.', MHTRP_DOMAIN ); ?>">[?]</a></th>
										<th><?php _e( 'Cost Per Item', MHTRP_DOMAIN ); ?> <a class="tips" data-tip="<?php _e( 'Additional cost per item.', MHTRP_DOMAIN ); ?>">[?]</a></th>
										<th><?php printf(__( 'Cost Per %s', MHTRP_DOMAIN ), $this->weight_unit); ?> <a class="tips" data-tip="<?php _e( 'Additional cost per weight unit.', MHTRP_DOMAIN ); ?>">[?]</a></th>
									</tr>
								</thead>
								<tfoot>
									<tr>
						   	...
						   	<script type="text/javascript">
						   		...
						   		foreach ($this->table_rates as $table_rate): 
						   			$table_rate['min'] = wc_format_localized_price($table_rate['min']);
						   			$table_rate['max'] = wc_format_localized_price($table_rate['max']);
						   			$table_rate['cost'] = wc_format_localized_price($table_rate['cost']);
						   			$table_rate['item_cost'] = wc_format_localized_price($table_rate['item_cost']);
						   			$table_rate['weight_cost'] = wc_format_localized_price($table_rate['weight_cost']);
				...
				function generate_shipping_class_order_table_html() {
					...
					<tr valign="top">
						...
						<td class="forminp" id="<?php echo $this->id; ?>_shipping_classes">
							<table class="wc_shipping widefat wp-list-table" cellspacing="0" style="width: 50%;">
								<tfoot>
									<tr>
										<th colspan="2"><span class="description"><?php _e( 'Drag and drop the above shipping classes to control their priority. Used only by <strong>per order</strong> and <strong>highest priority</strong> calculation types.', MHTRP_DOMAIN ); ?></span></th>
									</tr>
								</tfoot>
								<tbody>
<?php
...
?>
									<tr class="shipping_class">
										<td width="1%" class="sort"><input type="hidden" name="<?php echo $this->id; ?>_shipping_class_order[]" value="<?php echo esc_attr($slug); ?>" /></td>
										<td class="name"><?php echo esc_js($options[$slug]); ?></td>
									</tr>
<?php
		}
	}
...
?>
				...
				function process_table_rates() {
					...
					if ( isset( $_POST[ $this->id . '_table_rate_min'] ) )   $table_rate_min   = array_map( 'wc_format_decimal', $_POST[ $this->id . '_table_rate_min'] );
					if ( isset( $_POST[ $this->id . '_table_rate_max'] ) )   $table_rate_max   = array_map( 'wc_format_decimal', $_POST[ $this->id . '_table_rate_max'] );
					if ( isset( $_POST[ $this->id . '_table_rate_cost'] ) )  $table_rate_cost  = array_map( 'wc_format_decimal', $_POST[ $this->id . '_table_rate_cost'] );
					if ( isset( $_POST[ $this->id . '_table_rate_item_cost'] ) )  $table_rate_item_cost  = array_map( 'wc_format_decimal', $_POST[ $this->id . '_table_rate_item_cost'] );
					if ( isset( $_POST[ $this->id . '_table_rate_weight_cost'] ) )  $table_rate_weight_cost  = array_map( 'wc_format_decimal', $_POST[ $this->id . '_table_rate_weight_cost'] );
						...
						if ($zone['type'] == 'country') { // COUNTRY
							...
							if ($destination_country == $zone['country']) {
								...
								foreach ($postcodes as $postcode) {
									...
									if ($destination_postcode == $postcode) { // EXACT MATCH
									...
									} else if (in_array($destination_country, $uk_postcode_countries)) { // UK POSTCODES ########################

										if (preg_match("/^([A-Z]{1,2})$/i", $postcode, $matches)) { // UK POSTCODE AREA eg BN
										
											$pattern = $matches[1];
											$found = preg_match("/^$pattern([0-9]{1,2}[A-Z]?)$/i", $trimmed_destination_postcode);
										
										} else if (preg_match("/^([A-Z]{1,2}[0-9]{1,2}[A-Z]?)$/i", $postcode, $matches)) { // UK POSTCODE DISCTRICT eg BN1
										...
										} else if (preg_match("/^([A-Z]{1,2})([0-9]{1,2})-([0-9]{1,2})$/i", $postcode, $matches)) { // UK POSTCODE RANGE eg BN1-5
									...
									} else if ($destination_country == 'CA') { // CANADAIAN POSTCODES ###########################################
				...
				*/
				function get_available_table_rates($shipping_class, $calc_type) {
					
					$available_zones = $this->get_available_zones();
					...
					if ($calc_type == 'per_class') {
						...
						$weight 	= $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'weight' );
						$total 		= $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'total' );
						$item_count = $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'qty' );
					}
					...
					foreach ($this->table_rates as $table_rate):
						...
						$table_rate_class = $this->reverse_map_class($table_rate['class']);
						if ($table_rate_class == $shipping_class) {
							$class_pass = true;
						} else if ($table_rate_class == MHTRP_CLASS_ANY) {
							$class_pass = true;
						} else {
							$class_pass = false;
						}
				...
				*/
				function get_cart_highest_shipping_class() {
					...
					$found = false;
					$i = 0;
					$highest = MHTRP_CLASS_NONE;
					while (($found == false) && ($i < count($this->shipping_class_order))) {
					
						if (in_array($this->shipping_class_order[$i], $classes_found, true)) {
							$found = true;
				...
				*/
				function get_cart_stat_by_shipping_class($shipping_class, $stat) {
					...
					foreach ( $this->package['contents'] as $cart_item_key => $cart_item ) {
						...
						if ( ( ( ( $shipping_class === MHTRP_CLASS_ANY ) || ( $product_shipping_class === $shipping_class ) ) || ( ( $shipping_class === MHTRP_CLASS_NONE ) && empty( $product_shipping_class ) ) )
						&& $product->needs_shipping() ) {
							...
							if ($stat == 'weight') {
								...
								if ( $this->tax_mode == 'incl' ) {
									$total += $product->get_price_including_tax( $cart_item['quantity'] );
								} else {
									$total += $product->get_price_excluding_tax( $cart_item['quantity'] );
								}
				...
				/* 
					Check if a table rate for a shipping class has been set
				*/
				public function shipping_class_has_table_rate( $shipping_class ) {
				
					$found = false;
					$i = 0;
					while (($found == false) && ($i < count($this->table_rates))) {
						if ( $this->table_rates[$i]['class'] == $this->class_mapping[$shipping_class] ) {
							$found = true;
						} else {
							$i++;
						}
				...
				*/
				...
				/*
					Reverse map table rate class
				*/
				function reverse_map_class($class) {
					switch ($class) {
						case '0':
							return MHTRP_CLASS_ANY;
							break;
						case '-1':
							return MHTRP_CLASS_NONE;
							break;						
						default:
							return $class;
							break;
				...
				*/
				function get_cart_total() {
					$cart_total = $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'total');
					return apply_filters( 'mhtrp_cart_total', $cart_total );
				}
				...
				*/
				function get_handling_fee() {
					...
					// Trim and remove spaces from cost
					$handling_fee = wc_format_decimal($this->settings['handling_fee']);
					...
					if (!empty($handling_fee)
						...
						if (!empty($percentage) && is_numeric($percentage)) {
							$handling_fee = $this->get_cart_total() * ($percentage/100);
						}
					}
				...
				*/
				public function calculate_per_order() {
				
					$highest_shipping_class = $this->get_cart_highest_shipping_class();
				
					$available_table_rates = $this->get_available_table_rates($highest_shipping_class, 'per_order');
				...
				*/
				public function calculate_per_shipping_class() {
					...
					foreach ($shipping_classes as $shipping_class) {
						$table_rates[$shipping_class] = $this->get_available_table_rates($shipping_class, 'per_class');
					}
				...
				/* 
					Calculate for highest priority shipping class only. Called by calculate_shipping()
				*/
				public function calculate_highest_class_only() {
				
					$highest_shipping_class = $this->get_cart_highest_shipping_class();

					// If cart has no shipping classes revert to per_order calculation
					if ($highest_shipping_class == MHTRP_CLASS_NONE) {
						return $this->calculate_per_order();
					}
				
					$available_table_rates = $this->get_available_table_rates($highest_shipping_class, 'per_class');
				...
				*/
				public function calculate_tax_cost( $cost ) {

					if ($this->settings['tax_status'] == 'none') {
						$taxes = false;
					} else {
						if ( $this->tax_mode == 'incl' && wc_tax_enabled() ) {
					...
					return array( 'cost' => $cost, 'taxes' => $taxes );
				}
				...
				*/
				public function calculate_shipping( $package ) {
					...
					if ($this->settings['calc_type'] == 'per_class') {
						$table_rates = $this->calculate_per_shipping_class();
					} else if ($this->settings['calc_type'] == 'highest_class') {
						$table_rates = $this->calculate_highest_class_only();
					} else {
						$table_rates = $this->calculate_per_order();
					}

					// Get handling fee for all rates
					$handling_fee = $this->get_handling_fee();
					
					foreach ($table_rates as $table_rate):
					
						$cost 		= $table_rate['cost'] + $handling_fee;
						$tax_cost 	= $this->calculate_tax_cost( $cost );
						$service 	= $this->find_by_id($this->services, $table_rate['service']);
				...
				public function show_debug() {
					...
					$debug_msgs['Cart weight'] = $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'weight');
					$debug_msgs['Cart total'] = $this->get_cart_total();
					$debug_msgs['Cart item count'] = $this->get_cart_stat_by_shipping_class(MHTRP_CLASS_ANY, 'qty');

					$debug_msgs['Handling fee'] = $this->get_handling_fee();
